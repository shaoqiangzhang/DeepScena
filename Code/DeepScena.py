...
class DeepScena(nn.Module):
    ...
    def model_evaluation(self,first_phase):#model evaluation
    ...
    def first_phase(self):
      ...
      for T in range(0,self.MaxIter1):
        ...
        NMI, ARI = self.model_evaluation(first_phase=True)
        ARIlist.append(ARI)
        NMIlist.append(NMI)
    ...
    def second_phase(self):
        ...
        for T in range(0,self.MaxIter2):
            ...
            if T <= self.T2:
                self.u_mean = self.update_cluster_centers()
            ARI_prev=0.0
            NMI,ARI = self.model_evaluation(first_phase= False)
            NMIlist2.append(NMI)
            ARIlist2.append(ARI)
            if ARI> ARI_prev:
                ARI_prev = ARI
                with open('AE_Second_phase_' + self.dataset_name, 'wb') as f:
                    torch.save(self.AE, f)
                with open('MNet_Second_phase_' + self.dataset_name, 'wb') as f:
